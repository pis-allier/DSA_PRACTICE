 ...
 * 
 */
public class KeyPressDuration {

	public static void main(String[] args) {
		// TODO Auto-generated method stub

		int[] releTime = { 12, 23, 36, 46, 62 };
		String keyPressed = "spuda";
		int longestTime = longestTimePressed(releTime, keyPressed);
		 System.out.print((char)longestTime);
	}

	private static int longestTimePressed(int[] releaseTimes, String keysPressed) {
		// TODO Auto-generated method stub

		int maxi = releaseTimes[0]; // The duration of the first key press
		char lar = keysPressed.charAt(0); // The first key itself
		int n = releaseTimes.length; // The total number of key presses

		for (int i = 1; i < n; i++) {
			int diff = releaseTimes[i] - releaseTimes[i - 1]; // The duration of the current key press
			if (diff > maxi) {
				maxi = diff; // Update the maximum duration
				lar = keysPressed.charAt(i); // Update the slowest key
			} else if (diff == maxi && keysPressed.charAt(i) > lar) {
				lar = keysPressed.charAt(i); // Update the slowest key if it's lexicographically larger
			}
		}
