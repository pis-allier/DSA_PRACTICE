...
public class DistributeCandies {
	...
	private static int distributeCandies(int[] candyType) {
		// TODO Auto-generated method stub
		long canEatCandies = candyType.length / 2;
		Set<Integer> set = new HashSet<>();
		for (Integer x : candyType) {
			set.add(x);
		}
		long Uniquecount = set.stream().count();
		
       long max = Math.min(canEatCandies, Uniquecount);
       if(canEatCandies == Uniquecount)
    	   return (int) canEatCandies;
       else {
    	   return (int) max;
       }
	}
